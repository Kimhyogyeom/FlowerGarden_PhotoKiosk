...
/// <summary>
/// 특정 UI(RectTransform)를 기준으로
/// - 랜덤 X 위치 / 랜덤 스케일로 위쪽에서 등장해서
/// - 아래로 떨어지면서 좌우로 흔들리고 회전(나풀나풀)하는 효과를 주는 스크립트
/// - 한 사이클이 끝나면 옵션에 따라 자동으로 다시 반복
/// </summary>
public class FlutterDropFromHere : MonoBehaviour
{
    ...
    [Tooltip("현재 기준 위치에서 X축 최소 오프셋")]
    [SerializeField] private float _minXOffset = -200f;

    [Tooltip("현재 기준 위치에서 X축 최대 오프셋")]
    [SerializeField] private float _maxXOffset = 200f;

    [Tooltip("현재 Y에서 얼마나 더 위에서 시작할지 (0이면 현재 Y 그대로)")]
    ...
    [Tooltip("떨어지는 속도 랜덤 가감 값 (±값으로 사용)")]
    [SerializeField] private float _deducted = 100f;

    private float _originFallSpeed = 0f;

    [Tooltip("부모 Rect 아래에서 얼마나 더 내려가면 한 번의 떨어지기를 끝낼지(여분 마진)")]
    ...
    /// <summary>
    /// 컴포넌트가 추가될 때 기본 RectTransform 자동 할당용
    /// </summary>
    private void Reset()
    {
    ...
    private void Awake()
    {
        ...
        // 시각적 숨김에 사용할 CanvasGroup (필요시 주석 해제해서 사용)
    ...
    /// <summary>
    /// 자동 반복 루틴:
    /// - 랜덤 대기 → 한 번 떨어지기 → 완료 기다림 → 랜덤 대기 → (autoLoop면 반복)
    /// </summary>
    private IEnumerator LoopRoutine()
    {
        // 무한 루프(StopLoop()가 호출될 때까지)
        while (true)
        {
            // 1) 떨어지기 전에 랜덤 대기
            ...
            // 3) 떨어지는 동안 Update에서 _isPlaying이 false가 될 때까지 대기
            while (_isPlaying)
            {
                yield return null;
            }

            // 4) 한 사이클 끝나고 랜덤 추가 대기
    ...
    /// <summary>
    /// 한 번만 떨어지는 애니메이션을 위한 초기값 셋업 (위치/속도/랜덤 파라미터)
    /// </summary>
    private void PlayOnce()
    {
        ...
        // 속도 랜덤 (기본값 기준으로 ±_deducted 범위 내에서 랜덤)
        ...
        // 보이게 만들고 싶다면 CanvasGroup 활용
        //if (_hideWhenIdle && _canvasGroup != null)
        //    _canvasGroup.alpha = 1f;
    }

    /// <summary>
    /// 프레임마다 낙하/좌우 흔들림/회전을 적용하고,
    /// 화면 아래쪽 범위를 벗어나면 한 사이클을 종료
    /// </summary>
    private void Update()
    {
    ...
    /// <summary>
    /// 한 번 떨어지는 사이클 끝 처리
    /// - 현재는 재사용을 위해 상태 플래그만 변경
    /// - 실제 위치/각도 리셋은 다음 PlayOnce()에서 다시 설정
    /// </summary>
    private void FinishOneDrop()
    {
        _isPlaying = false;

        // 여기서 즉시 숨기고 싶으면 CanvasGroup 등을 이용해 처리 가능
        //if (_hideWhenIdle && _canvasGroup != null)
        //    _canvasGroup.alpha = 0f;
    }
}
